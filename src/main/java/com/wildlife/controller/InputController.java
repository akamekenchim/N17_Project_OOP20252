...
public class InputController {
    ...
    public static void StartListening(Scene scene, WorldMap map) {
        ...
        scene.setOnMouseClicked(event -> {
            ...
            double rawY = SimulationController.screenToWorldY(event.getY());
        ...
        scene.setOnScroll(event -> {
            ...
            double worldX_Before = (mouse_positionX - SimulationController.camX) / SimulationController.zoomLevel;
            double worldY_Before = (mouse_positionY - SimulationController.camY) / SimulationController.zoomLevel;
            double oldzoom = SimulationController.zoomLevel;
            if (scroll_y > 0) {
                SimulationController.zoomLevel = Math.min(5.0, SimulationController.zoomLevel + 0.1);
            }
            if (scroll_y < 0) {
                SimulationController.zoomLevel = Math.max(1.0, SimulationController.zoomLevel - 0.1);
                SimulationController.camX = 0;
                SimulationController.camY = 0;

                double nextCamX = SimulationController.camX;
                double nextCamY = SimulationController.camY;// + (deltaY / SimEngine.zoomLevel);

                double mapDisplayWidth = Constants.SCREEN_WIDTH * SimulationController.zoomLevel;
                double mapDisplayHeight = Constants.SCREEN_HEIGHT * SimulationController.zoomLevel;
                ...
                if (mapDisplayWidth > Constants.SCREEN_WIDTH) {
                    SimulationController.camX = Math.max(minCamX, Math.min(0, nextCamX));
                } else {
                    SimulationController.camX = 0;
                }
                if (mapDisplayHeight > Constants.SCREEN_HEIGHT) {
                    // double minY = Constants.SCREEN_HEIGHT - mapDisplayHeight;
                    SimulationController.camY = Math.max(minCamY, Math.min(0, nextCamY));
                } else {
                    SimulationController.camY = 0;
                }
            }
            if (oldzoom != SimulationController.zoomLevel) {
                SimulationController.camX = mouse_positionX - (worldX_Before * SimulationController.zoomLevel);
                SimulationController.camY = mouse_positionY - (worldY_Before * SimulationController.zoomLevel);
                isZoomed = SimulationController.zoomLevel > 1 ? true : false;
            }
        });
        ...
        scene.setOnMouseDragged(event -> {
            if (isZoomed) {
                ...
                double nextCamY = SimulationController.camY + (deltaY / SimulationController.zoomLevel);
